...
export default {
  ...
  methods: {
    ...
    async handleIndividualDrag() {
      ...
      this.taxAvoided.individual401k = (this.totalTaxBalance -
        (res?.taxBalance +
      ...
      this.taxAdvantageRatio.individual401k = Math.round(
        (this.taxAvoided.individual401k /
          (individual401kPersonal + individual401kBusiness)) *
        100
      );
    ...
    async handleSepIraDrag() {
      ...
      this.taxAvoided.sepIra = (this.totalTaxBalance - (res?.taxBalance + progressiveTax(this.userInput, ""))).toFixed(
        2
      );
    ...
    async handleSimpleIra() {
      ...
      let res = await repostData(simpleIraPersonal, simpleIraBusiness);
      this.taxAvoided.simpleIra = (this.totalTaxBalance -
      ...
      this.taxAdvantageRatio.simpleIra = Math.round(
        (this.taxAvoided.simpleIra /
          (simpleIraPersonal + simpleIraBusiness)) *
        100
      );
    ...
    async handleTraditionalIra() {
      ...
      this.taxAvoided.traditionalIra = (this.totalTaxBalance -
        (res?.taxBalance +
          progressiveTax(this.userInput, traditionalIraPersonal))
      ).toFixed(2);
      this.taxAdvantageRatio.traditionalIra = Math.round(
        (this.taxAvoided.traditionalIra / traditionalIraPersonal) * 100
      );
          ...
          description: [
            `Federal Tax avoided: ${span(
              this.totalFederalTax -
                federalIncomeTax -
                progressiveTax(this.userInput, this.sliders[`${name}Personal`])
            )}`,
